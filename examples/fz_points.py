...
z_fs = np.concatenate(
    [np.flip(z_meniscus_feed) + z_meniscus_crys.max(), z_meniscus_crys]
)
s_fs = np.concatenate(
...
# bottom
s_bt = np.array(
    [[0, -0.04], [1e-6, -0.04], [5e-6, -0.04], [0.05, -0.03], [0.075, -0.018], [0.1, 0]]
)
...
z_samples = (
    70 * (r_samples ** 2 - r_feed ** 2) + s_fs[0, 1]
)  # this leads to approx 3 mm interface deflection
s_mf = np.concatenate(
